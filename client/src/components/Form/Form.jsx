...
const Form = () => {
  ...
  const { accountName, currentUser, editMode, currentDiary } = useSelector(
    (state) => state.user
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        await axios.post(
          `${import.meta.env.VITE_API_URL}/api/account/diaries`,
          {
            ...formData,
            tags: formData.tags.split(",").map((tag) => tag.trim()),
  ...
  return (
    <div className="form">
      <div className="box2">
        <div className="login2">{editMode ? "EDIT DIARY" : "CREATE"}</div>
